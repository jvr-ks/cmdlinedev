...
guiMainCreate(hide := false) {
  ...
  global directoriesArr, paramMaxCount, Errormessage
  ...
  Menu, MainMenuTools, Add, Sort file clddirectories.txt (Name - alphabetically), clddirectoriesSort
  Menu, MainMenuTools, Add, Sort file clddirectories.txt (Latest - sort numerically), clddirectoriesSort2
  ...
  gui, guiMain:Add, ListView, x5 w%lv1Width% h%editheight%  GguiMainListViewEvent AltSubmit vLV hwndhLV Grid -Multi, |Name|Latest|Dir|Cmd1|Cmd2|Cmd3|Cmd4
  ; hidden global enter button 
  gui, guiMain:Add, Button, Default hidden gguiMainListViewEnter, Enter
...
guiMainListViewEnter(){
  global currentSelected, configFile
...
;---------------------------- getCurrentSelected ----------------------------
getCurrentSelected() {
  global directoriesArr, currentSelected, currentSelectedName
  
  Loop, % directoriesArr.Length() {
    directoryEntryArr := StrSplit(directoriesArr[A_Index],",")
    
    if (directoryEntryArr[1] = currentSelectedName){
      currentSelected := A_Index
    }
  }
...
;--------------------------------- LVModify ---------------------------------
LVModify(){
  global LV, hLV, currentSelected, currentSelectedName
  ...
  if (rowCount > 0){
    ...
    LV_ModifyCol()
    LV_ModifyCol(1,"AutoHdr Integer Sort SortAsc")
    ...
    if (currentSelectedName != ""){
      getCurrentSelected()
      LV_ModifyCol(1,"AutoHdr Integer Sort SortAsc")
      LV_Modify(currentSelected, "Select Vis Focus")
      LV_ModifyCol(1,"AutoHdr Integer")
      LV_ModifyCol(3,"AutoHdr Integer Sort SortDesc")
      
      GuiControl,, lastUsedText, %currentSelectedName%
     }
    GuiControl, +Redraw, LV
    SendMessage, 0x115, 6, 0, , ahk_id %hLV%
  }
}
;---------------------------------- WM_MOVE ----------------------------------
...
guiMainGuiSize(){
  ...
  global borderLeft, borderRight, borderTop, LV, appIsMinimized
  ...
  if (A_EventInfo != 1) {
    ...
    guicontrol, guiMain:move, LV, x%borderLeft% w%width% h%height%
