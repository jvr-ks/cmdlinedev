...
------------ | ------------- | -------------  
entry is not editable, if it is empty | bug | 0.031
open the Document List sometimes fails | issue | ---
~~App fails if minimized instead of unfocused~~ | bug | 0.317  
~~"!opensession!#NAME" opening default entry~~ | issue | 0.294  
  
#### Latest changes:  
  
Version (&gt;=)| Change  
------------ | -------------  
0.333 | Fileencoding chanced to UTF-8 (with BOM) (all \*.txt and sourcecode-files!)
0.332 | **Configuration-file renamed to: "cldConfig"**, Sourcecode rearranged
0.331 | All builtin-functions renamed to "!Builtin-functions!"
0.330 | Run external app (Default Hotkey: \[ALT] + \[r])
0.328 | Open session mechanism was made more reliable, session list usage is independent of sorting
0.322 | Two new configuration parameters
0.322 | Latest-field resolution reduced to 10 seconds  
0.322 | Use a Double-Click (instead of a single clicks) to start the operation(s)  
0.320 | Edit mask enhanced (\[Shift] + \[Click)]), FastSwitch-Cycle disabled by default (switched to the Windows standard.)  
0.318 | **Main table format changed (see below ...)**  
0.316 | Right-Click (or \[Ctrl] + \[Shift] + \[Click)] to set the line entry as current (move to the top)  
0.315 | Besides a click the "Enter" button may be used to run the currently focused line entry   
0.314 | Clicking the Close-Cross really kills the app!  
0.306 | FS Auto Cycle Learn: Hidden windows (system processes) are suppressed by default  
0.306 | A32 ANSI version removed  
0.303 | FS AUto Cycle Edit: Bugfixes  
0.299 | Forced open of document-list (all files contained in the session) (uses Ctrl+Tab)  
0.295 | Configuration-file -&gt; \[external] texteditorpath was renamed to notepadppPath  
0.294 | Configuration-file -&gt; "cmdlinedev_COMPUTERNAME.ini" takes precedence over "cmdlinedev.ini"  
0.294 | Running multible notepad++-sessions is allowed, each window has a distinct id (made of date-time)  
0.293 | Opensession enhanced, coordinates dependencies removed  
0.291 | Menu-entry: Help  
0.290 | Configuration-file -&gt; gui parameter moved to section \[gui] (manually remove previous ones)  
0.289 | "actual URL mechanism" added  
0.287 | "°"-character inside URLs usable as an alias of the "#"-character, Refreshing the list after an "Edit Directories-file"  command  
...
I just never used them and I never used the "Fastswitch-Cycle feature"!  
"FS Auto Cycle" is the better choice!  
So I disabled (commented out) the hotkeys (file: "cldfastswitchhotkeys.txt") and  
set all "fastSwitchCycleGroups_Off" (Configuration-file).  
  
### Changes from version &gt;= 0.323  
Two new configuration parameters used:  
Configuration-file -&gt; \[config] -&gt; sessionfiles="C:\npp-sessions\"  
Configuration-file -&gt; \[config] -&gt; sessionfileExtension=".npp-session"  
These parameters must be set once!  

### Changes from version &gt;= 0.322  
Use a Double-Click (instead of a single click) to start the operation(s)! 

### Main table format changed (from version &gt; 0.317)  
Main table format changed (file "clddirectories.txt", an additional column inserted after column 1).  
...
**Up or down makes no difference, the entry is allways moved to the top!** (By setting the "Latest"-field).  
  
The "Latest"-field can be set manually also (Edit: SHIFT + Double-Click).  
...
(If the Configuration-file contains Unicode characters, it should use UTF-16 LE BOM).  
Use [Notepad\+\+](https://notepad-plus-plus.org/), Atom, Scite or any other editor to edit the files:   
  
Configuration-file **"cmdlinedev.ini"** or **"cmdlinedev_COMPUTERNAME.ini"**  
Directories-file **"clddirectories.txt"**,  
Tools-file **"cldtools.txt"**,  
Shortcuts-file **"cldshortcuts.txt"**,   
  
#### Configuration-file:  
The default Configuration-file filename is "cmdlinedev.ini",  
overriden by "cmdlinedev_COMPUTERNAME.ini", if it exists.  
The Cmdlinedev reads and stores data to the Configuration-file.  
...
**Command modifier, holding down key while clicking:**  
...
\[Alt] + \[Ctrl] | move the line entry to top \*1)   
\[Alt] + \[Win]+ \[Ctrl] | move the line entry one position down \*2)  
  
\*1)  
If the "Latest"-field has a value:  
* Moves the line entry to the top inside the Directories-file,  
* Has no visible effect.  
  
If the "Latest"-field has no value:  
* Moves the line entry to the top inside the Directories-file,  
* Moves the line entry to the top of the table, but allways below all rows with a non-empty "Latest"-field.
  
\*2)  
If the "Latest"-field has a value:  
* Moves the line entry one position down inside the Directories-file,  
* Has no visible effect.  
  
If the "Latest"-field has no value: 
* Moves the line entry one position down inside the Directories-file,  
* Moves the line entry one position down of the table.
...
#### 1) Fastswitch hotkeys (deprecated)  
...
Besides using one of the two fast-switch modes, each hotkey can be used directly too,  
i.e. as an example: pressing \[Alt] + \[b] opens the firefox browser!  
...
with:  
^+#!b,Mozilla Firefox  
Then edit the Configuration-file and replace in each entry fastSwitchCycleGroup1 ... fastSwitchCycleGroup9  
"!b" with "^+#!b"  
Looks ugly and if you want to use the hotkey directly, you have to press simultaneously:   
\[CTRL] + \[SHIFT] + \[WIN] + \[ALT],  
which is difficult to do, unless you are an alien!   
...
To cycle throught multible apps-windows in a defined order with only one hotkey.  
Example:  
The fastSwitchCycle-section of the Configuration-file contains the following definition:   
\[fastSwitchCycle]  
fastSwitchCycleHotkey="Alt & q"  
Overrides default Windows operation!  
Activate Caps Lock to disable the override temporarily!  
  
fastSwitchCycleGroup2="!f,!n"  
fastSwitchCycleGroup3="!b,!n"  
fastSwitchCycleGroup4=...  
up to fastSwitchCycleGroup9  
   
The groups (2 - 9)(up to 8) are selectable below the menu via radio-buttons.  
(Needs a reload of Cmdlinedev after a change!)  
The hotkeys are defined in the Fastswitch-file "cldfastswitchhotkeys.txt",  
they are also directly usable, i.e. independant of the Cycle-Groups.  
  
Drawbacks:  
- Hotkeys used by Cmdlinedev cannot be used by other apps anymore.  
...
Pressing the button FSS Auto Cycle "Learn" adds running-apps-windows to the cycle-list.  
...
[Virusscan at Virustotal, cmdlinedev.exe 64bit-exe, Check here](https://www.virustotal.com/gui/url/48d0cae565c0027012898f66ec70aaddba01bc94d9b91d68c82361bbfe4b5414/detection/u-48d0cae565c0027012898f66ec70aaddba01bc94d9b91d68c82361bbfe4b5414-1711028250
)  
[Virusscan at Virustotal, cmdlinedev32.exe 32bit-exe, Check here](https://www.virustotal.com/gui/url/86226d55d68d26a8dcaa92d608d573a232fc4cd2dabc3482da2ba937f07c930e/detection/u-86226d55d68d26a8dcaa92d608d573a232fc4cd2dabc3482da2ba937f07c930e-1711028252
)  
