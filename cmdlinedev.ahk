...
#Include cmdlinedev_mainWindow.ahk
...
configFile := "cldConfig" . ".ini"
configFileLocal := "cldConfig" . "_" . A_ComputerName . ".ini"

if (!FileExist(configFile)){
  createDefaultConfig(configFile) ; creates the missing configFile, configFileLocal is never automatically created
}
...
if (hideOnStartup){
  tipTop(app . " started,`nHotkey is: " . hotkeyToText(menuHotkey) . ",`nConfiguration-file:`n" . configFile . ",`nDirectories-file:`n" . directoriesFile,1, 5000)
  prepare()
  readGuiParam()
  mainWindow(true)
} else {
...
guiMainListViewEvent(){
  global directoriesFile, directoriesArr, currentSelected
...
runInDir(lineNumber) {
  ...
  global paramMaxCount, forcedCancel, emptyFiledSubstituteChar
...
runInDirDefault(i,useonly){
  ...
  global configFile, runnerPath, Edit1, emptyFiledSubstituteChar
    ...
    if (command != ""){
      ...
      if (tool != ""){
        ...
        switch c
        {
          ...
          case "!externalApp!":
            if (parameter1 != ""){
              IniWrite, %parameter1%, %configFile%, external, runnerPath
              ControlSetText, Edit1, %parameter1%
              runnerPath := parameter1
            }
            
          case emptyFiledSubstituteChar:
