...
/*
 *********************************************************************************
 * Main view element is the ListView LV.
...
;-------------------------------- saveGuiData -------------------------------
saveGuiData(){
  global mainHWND, configFile, windowPosX, windowPosY, clientWidth, clientHeight, currentSelectedName
  
  ;IniWrite, %currentSelected%, %configFile%, gui, currentSelected
  IniWrite, %currentSelectedName%, %configFile%, gui, currentSelectedName
...
guiMainListViewEvent(){
  global directoriesFile, directoriesArr, currentSelected, configFile
  ; LV_GetText(tmp, A_EventInfo)
  ; tooltip A_GuiEvent: %A_GuiEvent% A_EventInfo: %A_EventInfo% content %tmp%

  if (A_GuiEvent = "Normal"){
    LV_GetText(currentSelected, A_EventInfo)
...
runInDir(lineNumber) {
  ...
  global lastUsedText, currentSelected, currentSelectedName, eventTriggered
  global pathSelected
  ...
  currentSelectedName := directoryEntryArr[1]
